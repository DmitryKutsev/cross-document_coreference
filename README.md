...
Самая базовая версия алгоритма неожидонно показала следующий результат на размеченных данных:
md``` 
0.5663265306122449  presicion
0.6166666666666667  recall
0.5904255319148937  f-score```

Проверим, как алгоритм работает не на размеченных данных, а только на местоимениях. Если раньше в качестве признака для извлечения анафоров, мы использовали разметку класса, то сейчас попробуем извлекать их, используя часть речи.
...
Модели, обученные на датасете, а именно Decision trees, Random forest, k-nearest neighbors, SVD, линейная регрессия, не показали результатов, превышающий бейзлайн, и в среднем колебались на значении `0.5 presicion`, и ниже.
