...
# Переехал в гугл-док https://docs.google.com/document/d/1TtxMLPjB6tCr1aPt8kW2vqxmTgPZtUXM5F-wRXYV_jo/edit#


## Оглавление.
Введение
1. Анафора и методы разрешения анафоры
2. Предыдущие исследования анафоры и кореференции
3. Данные
...
## Анафора и методы разрешения анафоры.
...
## Предыдущие исследования анафоры и кореференции.
...
Внутри форумов были выбраны подфорумы. "Вопросы по препаратам" для hv-info, "Чем и как лечимся" для rakpobedim и "Психофармакотерапия: обсуждение препаратов, отзывы" для форума neuroleptic. Данные с форума mneploho.net не размечались, но были использованы в качестве обучения векторных представлений слов, как тематически близкие.
 
Форумы структурированы иерархически: каждый форум делится на подфорумы, подфорумы на топики, в каждом топике имеется определенное количество постов. Посты - это пользовательские сообщения по теме, которая прикрепляется в самом первом сообщении топика. Как правило, тема содержит название обсуждаемого препарата. Например, на форуме neuroleptic из 40 просмотренных топиков в 35 название препарата, обсуждаемого в теме, присутствовало в первом сообщении. Из оставшихся пяти названия препаратов присутствовали в большинстве постов. Например, в теме "Посоветуйте активирующий нейролептик" ответами являются посты с названиями препаратов.
 
В зависимости от тематики, форумы более, или менее четко структурированы, например форум с обсуждениями препаратов против рака имеет менее четкую тематическую структуру, чем форум с обсуждениями нейролептиков.
 
Отдельно стоит отметить данные форума, посвященного препаратам против гепатита. В большинстве постов присутствует множество сленговых фраз и выражений, что само по себе может составлять отдельную тему для исследования. Ниже представлены примеры сленговых выражений и их расшифровки:
...
Вокс - воксилапревир
Рибы - рибавирин.
Трансы - трансаминазы (АЛТ, АСТ) э(а не тыц-тыц-тыц, как некоторые думают :t: )
...
В обсуждениях нейролептриков и препаратов против рака также присутствует своя терминология, но она либо интуитивно понятна, либо представляет собой набор медицинских терминов.
...
```
  ...
  ```
В разделе с обзором предыдущих работ присутствует краткое описание морфологических и синтаксических признаков, используемых в работах по разрешению анафоры. Пол и число были добавлены в датафрейм, как признаки, по которым можно проверить согласование антецедента и анафора. В процессе исследования корпуса оказалось, что  это далеко не всегда применимо к медицинскому пользовательскому контенту. Рассмотрим такой пример:

```
Вся моя "болезнь" и началась с паксила. Полсе первой таблетки началис судроги паника, отказало глотание , речь. 
Таблетки пить претсала а состояние отсалось.
```
Здесь таблетки и паксил имеют анафорическую связь (таблетки - анафор, паксил - антецедент), однако не согласуются по полу и числу. Такие случаи характерны, когда анафор выражен существительным. В случае с наречиями, как правило, согласование присутствует.

Часть речи, одушевленность, и падеж были добавлены, чтобы исследовать возможность их влияния на разрешение анафоры в процессе изучения моделей.

Как и обозначено выше, в качестве синтаксического парсера использовалась библиотека Spacy. Парсер размечал каждое предложение, выделяя корень предложения, и разбивал на фразы, выделяя главные и зависимые слова, а также размечая тип зависимости, таким образом формируя синтаксическое дерево. На рис.1 представлен пример разметки Spacy.

![рис.1](https://github.com/DmitryKutsev/cross-document_coreference/blob/main/images/displacy.jpg)

Интуиция извлечения информации о главном слове, и его зависимых, состоит в том, чтобы таким образом точечно извлечь контекст. Такой метод встречается не часто, и вероятно будет не информативен, тем не менее его можно исследовать.
Основной же контекст извлекался в качестве вектора всего предложения.
...
## Модели Переехали в гугл-док https://docs.google.com/document/d/1TtxMLPjB6tCr1aPt8kW2vqxmTgPZtUXM5F-wRXYV_jo/edit#
...
В исследовании как для векторизации текста, так и в качестве классификаторов используются различные модели машинного обучения. Один из наиболее распространенных и простых алгоритмов для классификации - K-nearest Neighbors [32]. Классификация здесь происходит за счет вычисления Евклидового расстояния между объектом, и его ближайшими соседями, число которых, k - задается при обучении, в матрице нормализованных признаков.

Не менее распространенным алгоритмом для классификации является Дерево Решений (Decision Tree) [33]. Дерево решений представляет собой граф, ребра которого сравниваются с ветками дерева, а узлы - с листьями. В каждом узле дерева решений содержится условие, ветки разделяют признаки на удовлетворяющие условию, и не удовлетворяющие. На каждом шаге выбирается признак, получающий максимальное количество информации, что максимально уменьшает уровень неопределенности данных при разделении признаков. Для этого вводится понятие энтропии, измеряющее этот уровень неопределенности, это сумма отрицательных значений вероятностей для всех рассматриваемых классов. Другими словами, когда с учетом каждого признака распределение по классам будет равновероятно, энтропия будет максимальной.
...
### Применение правилового метода
На рисунке (рис.2) представленно, как это выглядит в изначальном варианте. Стоит также отметить на рисунке 2 такое явление, как катафора. это разновидность анафоры, в которой анафор находится перед антецедентом. Подробно в данном исследовании катафора не рассматривается.

![рис.2](https://github.com/DmitryKutsev/cross-document_coreference/blob/main/images/annts.jpg)
`рис.2`


Здесь первый пост топика - **Обсуждаем препарат Трускал (Хлорпротиксен)**. Этот пост состоит из одного предложения, и содержит антецедент - Трускал.
В первом предложении следующего поста присутствует анафор, выраженный существительным - **препарат**. В этом предложении нет подходящих антецедентов, как и в предложениях внитри поста до него, и мы связываем его с антецедентом самого первого поста топика анафорической связью. Во втором предложении присутствует анафор **он**. И снова, его можно связать с антецедентом из первого сообщения топика, ввиду отсутствия других кандидатов в том же предложении, или посте. В третьем предложении анафоров нет, но есть антецедент, **трускал**. В четвертом предложении есть анафор **него**. В том же предложении антецедентов нет, и он связывается с антецедентом из предыдущего предложения.

В процессе разметки было обнаружено, что похожая логика часто повторяется. В данной работе мы попробуем проверить, будет-ли это большинством случаев. Для этого эксперимента был построен простой наивный алгоритм, проводящий разрешение анафоры, основываясь на структуре данных пользовательского контента в рамках форумов.
...
На рисунке ниже (рис.3) блок-схема самой простой реализации данного алгоритма.

![рис.3](https://github.com/DmitryKutsev/cross-document_coreference/blob/main/images/alg.jpg)
`рис.3`

У данного метода, в силу его простоты, много недостатков. Один из самых очевидных - предложения, не содержащие антецедент, относящийся к медицинским препаратам в топике, но связанные с антецедентом, который к ним не относится. Например:
```Мой друг пил феназепам несколько месяцев. Его состояние только ухудшилось, и теперь он не может писать диплом.```
Такие случаи можно отлавливать, включая в алгоритм соответствующие паттерны. Однако, на удивление, таких случаев в процессе разметки почти не обнаружилось. Есть вероятность, что это можно отнести к специфике дискурса, но не стоит исключать и ошибки в процессе разметки.
...
```
Где-то года три назад начал принимать этаперазин с циклодолом. Тогда в жизни еще были какие-то краски, мог подолгу разговаривать, строить планы на будущее. Теперь же превратился в тупого овощя, удовольствие доставляет только еда и сон, и то все как то тупо. Разговаривать практически разучился, соображаю тоже с большим трудом. От эмоций практически ничего не осталось, иногда смеюсь, но тоже как то по-идиотски. И кажется, что прежнюю жизнь уже не вернуть. Может быть все же я ошибаюсь. Просто уже несколько раз бросал пить таблетки, однако ничего не менялось, иногда становилось даже хуже из-за того, что возвращалась тревога и беспокойство и снова приходилось возобновлять прием. Или это состояние связано с моим заболеванием?
  О себе: 32
```
...
[31] https://link.springer.com/article/10.1186/s12859-018-2543-1 Bidirectional long short-term memory with CRF for detecting biomedical event trigger in FastText semantic space Yan Wang, Jian Wang, Hongfei Lin, Xiwei Tang, Shaowu Zhang & Lishuang Li  

[32] E. Fix and J.L. Hodges (1951): An Important Contribution to Nonparametric Discriminant Analysis and Density Estimation

[33] https://link.springer.com/content/pdf/10.1007/BF00116251.pdf Induction of Decision Trees J.R. QUINLAN
