...
Как и обозначено выше, в качестве синтаксического парсера использовалась библиотека Spacy. Парсер размечал каждое предложение, выделяя корень предложения, и разбивал на фразы, выделяя главные и зависимые слова, а также размечая тип зависимости, таким образом формируя синтаксическое дерево. Вот пример разметки Spacy:

![alt text](https://github.com/DmitryKutsev/cross-document_coreference/blob/main/images/displacy.jpg)
